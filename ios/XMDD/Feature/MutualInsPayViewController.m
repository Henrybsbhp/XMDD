...
- (NSString *)calcCouponTitle:(NSArray *)couponArray
{
    ...
    else if (couponArray.count == 1)
    {
        ...
        if (coupon.couponAmount >= self.maxCouponAmt)
        {
            CGFloat totalAmount = self.maxCouponAmt;
...
- (BOOL)callPaymentHelperWithPayOp:(PayCooperationContractOrderOp *)op
{
    ...
    switch (op.req_paychannel) {
        ...
        case PaymentChannelUPpay: {
            ...
            [helper resetForUPPayWithTradeNumber:op.rsp_tradeno andPayInfoModel:op.rsp_payInfoModel andTotalFee:self.priceLb.text.floatValue targetVC:self];
        } break;
