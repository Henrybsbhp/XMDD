...
- (NSInteger)numberOfPages {
    ...
    NSInteger num = [self.dataSource numberOfPagesForPaginatorView:self];
    return num;
}
...
-(void)initializeWithFrame:(CGRect)frame
{
    ...
    // Scroll view
    _scrollView = [[SYPaginatorScrollView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
    ...
    _pageControl = [[SYPageControl alloc] initWithFrame:CGRectMake(0.0f, size.height - 15.0f, size.width, 15.0f)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    [self scrollVisibleToCorrectView];
...
- (void)scrollVisibleToCorrectView
{
    ...
    if ([self numberOfPages] > 2)
    {
        ...
        {
            self.currentPageIndex = [self numberOfPages] - 2;
            [self.scrollView scrollRectToVisible:CGRectMake(([self numberOfPages] - 2) * self.scrollView.bounds.size.width, 0, self.scrollView.bounds.size.width, self.scrollView.bounds.size.height) animated:NO];
        }
        else if (self.currentPageIndex == self.numberOfPages - 1)
        {
            self.currentPageIndex = 1;
            [self.scrollView scrollRectToVisible:CGRectMake(self.scrollView.bounds.size.width, 0, self.scrollView.bounds.size.width, self.scrollView.bounds.size.height) animated:NO];
        }
    }
}
