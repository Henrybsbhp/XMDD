...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.watermarkText.length > 0) {
        ...
        UIFont *textFont = self.watermarkTextFont ? self.watermarkTextFont : [UIFont boldSystemFontOfSize:30];
        NSAttributedString *attrStr = [self attributedStringForWatermarkTextWithFont:textFont];
        CGSize  textSize = [attrStr size];
        CGContextTranslateCTM(ctx, rect.size.width/2.0, rect.size.height/2.0);
        CGContextRotateCTM(ctx, -M_PI_4);
        [attrStr drawAtPoint:CGPointMake(floor(-textSize.width/2.0), floor(-textSize.height/2.0))];
    }
}

- (NSAttributedString *)attributedStringForWatermarkTextWithFont:(UIFont *)font
{
    UIColor *textColor = self.watermarkTextColor ? self.watermarkTextColor : [[UIColor darkGrayColor] colorWithAlpha:0.75];
    ...
    return [[NSAttributedString alloc] initWithString:self.watermarkText attributes:dict];
}
